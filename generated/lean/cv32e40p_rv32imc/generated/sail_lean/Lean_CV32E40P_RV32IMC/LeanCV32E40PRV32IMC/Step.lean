...
import LeanCV32E40PRV32IMC.Common
import LeanCV32E40PRV32IMC.RvfiDii
...
import LeanCV32E40PRV32IMC.InstsEnd
import LeanCV32E40PRV32IMC.StepCommon
import LeanCV32E40PRV32IMC.StepExt
import LeanCV32E40PRV32IMC.DecodeExt
import LeanCV32E40PRV32IMC.Fetch
...
/-- Type quantifiers: k_ex163060_ : Bool, step_no : Int -/
...
/-- Type quantifiers: k_ex163110_ : Bool, step_no : Nat, 0 ≤ step_no -/
def try_step (step_no : Nat) (exit_wait : Bool) : SailM Bool := do
  ...
  | .Step_Execute (.Retire_Success (), _) =>
    assert (hart_is_active (← readReg hart_state)) "model/postlude/step.sail:202.74-202.75"
  ...
  | .Step_Execute (.Enter_Wait wr, instbits) =>
    (do
      ...
      then assert (hart_is_active (← readReg hart_state)) "model/postlude/step.sail:210.41-210.42"
      else
        (do
          if ((get_config_print_instr ()) : Bool)
          then
