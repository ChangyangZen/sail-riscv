...
def __WriteRAM_Meta (addr : (BitVec (if ( 32 = 32  : Bool) then 34 else 64))) (width : Nat) (meta' : Unit) : Unit :=
  ()

/-- Type quantifiers: width : Nat, 1 ≤ width ∧ width ≤ 4096 -/
def __ReadRAM_Meta (addr : (BitVec (if ( 32 = 32  : Bool) then 34 else 64))) (width : Nat) : Unit :=
  default_meta
