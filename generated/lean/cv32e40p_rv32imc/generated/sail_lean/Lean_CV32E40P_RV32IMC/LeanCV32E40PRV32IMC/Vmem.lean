...
import LeanCV32E40PRV32IMC.VmemPte
import LeanCV32E40PRV32IMC.VmemPtw
...
/-- Type quantifiers: k_ex154110_ : Bool, level : Nat, k_ex154108_ : Bool, k_ex154107_ : Bool, sv_width
...
def pt_walk (sv_width : Nat) (vpn : (BitVec (sv_width - 12))) (ac : (AccessType Unit)) (priv : Privilege) (mxr : Bool) (do_sum : Bool) (pt_base : (BitVec (if ( sv_width
  ...
  assert ((sv_width == 32) || (xlen == 64)) "model/sys/vmem.sail:103.36-103.37"
  ...
  then (pure Bare)
    ...
    (do
      ...
      let mbits ← (( do
        ...
        | RV64 =>
          (do
            assert (xlen ≥b 64) "model/sys/vmem.sail:200.25-200.26"
            throw Error.Exit)
        | RV32 =>
          (pure (0b000#3 ++ (_get_Satp32_Mode
                (Mk_Satp32 (Sail.BitVec.extractLsb (← readReg satp) 31 0)))))
        | RV128 => (internal_error "model/sys/vmem.sail" 204 "RV128 not supported") ) : SailM
        satp_mode )
      match (satpMode_of_bits arch mbits) with
      | .some m => (pure m)
      | none => (internal_error "model/sys/vmem.sail" 209 "invalid translation mode in satp"))

/-- Type quantifiers: k_ex154172_ : Bool, k_ex154171_ : Bool, sv_width : Nat, pte_size : Nat, pte_size
  ≥ 0, is_sv_mode(sv_width) ∧ pte_size ∈ {4, 8} -/
def translate_TLB_hit_pte (sv_width : Nat) (asid : (BitVec (if ( 32 = 32  : Bool) then 9 else 16))) (vpn : (BitVec (sv_width - 12))) (ac : (AccessType Unit)) (priv : Privilege) (mxr : Bool) (do_sum : Bool) (ext_ptw : Unit) (ent : TLB_Entry) (pte_size : Nat) (pte : (BitVec (pte_size * 8))) : SailM (Result ((BitVec (if ( sv_width
        ...
        (do
          ...
          then (pure (Err ((PTW_PTE_Update ()), ext_ptw)))
            ...
            (do
              ...
              | .Ok _ => (pure ())
              | .Err e =>
                (internal_error "model/sys/vmem.sail" 259 "invalid physical address in TLB")
...
/-- Type quantifiers: k_ex154219_ : Bool, k_ex154218_ : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
def translate_TLB_hit (sv_width : Nat) (asid : (BitVec (if ( 32 = 32  : Bool) then 9 else 16))) (vpn : (BitVec (sv_width - 12))) (ac : (AccessType Unit)) (priv : Privilege) (mxr : Bool) (do_sum : Bool) (ext_ptw : Unit) (ent : TLB_Entry) : SailM (Result ((BitVec (if ( sv_width
  = 32  : Bool) then 22 else 44)) × Unit) (PTW_Error × Unit)) := do
  let pte_size :=
    if ((sv_width == 32) : Bool)
    then 4
...
/-- Type quantifiers: k_ex154263_ : Bool, k_ex154262_ : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
...
/-- Type quantifiers: k_ex154299_ : Bool, k_ex154298_ : Bool, sv_width : Nat, is_sv_mode(sv_width) -/
...
def get_satp (sv_width : Nat) : SailM (BitVec (if ( sv_width = 32  : Bool) then 32 else 64)) := do
  assert ((sv_width == 32) || (xlen == 64)) "model/sys/vmem.sail:391.30-391.31"
  let satp_width := 32
  let satp_value ← do (pure (BitVec.toNatInt (Sail.BitVec.extractLsb (← readReg satp) 31 0)))
...
def translateAddr (vAddr : virtaddr) (ac : (AccessType Unit)) : SailM (Result (physaddr × Unit) (ExceptionType × Unit)) := do
  ...
  then (pure (Ok ((Physaddr (zero_extend (m := 34) (bits_of_virtaddr vAddr))), init_ext_ptw)))
    ...
    (do
      ...
      assert ((sv_width == 32) || (xlen == 64)) "model/sys/vmem.sail:419.36-419.37"
