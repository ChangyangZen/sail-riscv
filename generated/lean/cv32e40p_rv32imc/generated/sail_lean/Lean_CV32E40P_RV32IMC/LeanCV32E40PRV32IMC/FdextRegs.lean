...
import LeanCV32E40PRV32IMC.FregType
...
def freg_write_callback (x_0 : fregidx) (x_1 : (BitVec (if ( false  : Bool) then 8 else 4 * 8))) : Unit :=
  ()

def dirty_fd_context (_ : Unit) : SailM Unit := do
  assert (hartSupports Ext_F) "model/extensions/FD/fdext_regs.sail:110.28-110.29"
...
def dirty_fd_context_if_present (_ : Unit) : SailM Unit := do
  assert (neq_bool (hartSupports Ext_F) (hartSupports Ext_Zfinx)) "model/extensions/FD/fdext_regs.sail:117.55-117.56"
  if ((hartSupports Ext_F) : Bool)
  then (dirty_fd_context ())
  else (pure ())

def rF (app_0 : fregno) : SailM (BitVec (if ( false  : Bool) then 8 else 4 * 8)) := do
  let .Fregno r := app_0
  assert (hartSupports Ext_F) "model/extensions/FD/fdext_regs.sail:122.28-122.29"
  let v ← (( do
    match r with
    | 0 => readReg f0
    | 1 => readReg f1
...
def wF (typ_0 : fregno) (in_v : (BitVec (if ( false  : Bool) then 8 else 4 * 8))) : SailM Unit := do
  let .Fregno r : fregno := typ_0
  assert (hartSupports Ext_F) "model/extensions/FD/fdext_regs.sail:163.28-163.29"
...
def rF_bits (app_0 : fregidx) : SailM (BitVec (if ( false  : Bool) then 8 else 4 * 8)) := do
  let .Fregidx i := app_0
  (rF (Fregno (BitVec.toNatInt i)))

def wF_bits (typ_0 : fregidx) (data : (BitVec (if ( false  : Bool) then 8 else 4 * 8))) : SailM Unit := do
...
def rF_H (i : fregidx) : SailM (BitVec 16) := do
  assert (flen ≥b 16) "model/extensions/FD/fdext_regs.sail:224.19-224.20"
  assert ((hartSupports Ext_F) && (not (hartSupports Ext_Zfinx))) "model/extensions/FD/fdext_regs.sail:225.59-225.60"
  (pure (nan_unbox (m := 16) (← (rF_bits i))))

def wF_H (i : fregidx) (data : (BitVec 16)) : SailM Unit := do
  assert (flen ≥b 16) "model/extensions/FD/fdext_regs.sail:231.19-231.20"
  assert ((hartSupports Ext_F) && (not (hartSupports Ext_Zfinx))) "model/extensions/FD/fdext_regs.sail:232.59-232.60"
  (wF_bits i (nan_box (n := (4 *i 8)) data))

def rF_S (i : fregidx) : SailM (BitVec 32) := do
  assert (flen ≥b 32) "model/extensions/FD/fdext_regs.sail:238.19-238.20"
  assert ((hartSupports Ext_F) && (not (hartSupports Ext_Zfinx))) "model/extensions/FD/fdext_regs.sail:239.59-239.60"
  (pure (nan_unbox (m := 32) (← (rF_bits i))))

def wF_S (i : fregidx) (data : (BitVec 32)) : SailM Unit := do
  assert (flen ≥b 32) "model/extensions/FD/fdext_regs.sail:245.19-245.20"
  assert ((hartSupports Ext_F) && (not (hartSupports Ext_Zfinx))) "model/extensions/FD/fdext_regs.sail:246.59-246.60"
  (wF_bits i (nan_box (n := (4 *i 8)) data))

def rF_D (i : fregidx) : SailM (BitVec 64) := do
  assert (flen ≥b 64) "model/extensions/FD/fdext_regs.sail:252.19-252.20"
  throw Error.Exit

def wF_D (i : fregidx) (data : (BitVec 64)) : SailM Unit := do
  assert (flen ≥b 64) "model/extensions/FD/fdext_regs.sail:259.19-259.20"
  throw Error.Exit

def rF_or_X_H (i : fregidx) : SailM (BitVec 16) := do
  assert (flen ≥b 16) "model/extensions/FD/fdext_regs.sail:271.19-271.20"
  assert (neq_bool (hartSupports Ext_F) (hartSupports Ext_Zfinx)) "model/extensions/FD/fdext_regs.sail:272.55-272.56"
  if ((hartSupports Ext_F) : Bool)
  then (rF_H i)
  else (pure (Sail.BitVec.extractLsb (← (rX_bits (fregidx_to_regidx i))) 15 0))

def rF_or_X_S (i : fregidx) : SailM (BitVec 32) := do
  assert (flen ≥b 32) "model/extensions/FD/fdext_regs.sail:280.19-280.20"
  assert (neq_bool (hartSupports Ext_F) (hartSupports Ext_Zfinx)) "model/extensions/FD/fdext_regs.sail:281.55-281.56"
  if ((hartSupports Ext_F) : Bool)
  then (rF_S i)
  else (pure (Sail.BitVec.extractLsb (← (rX_bits (fregidx_to_regidx i))) 31 0))

def rF_or_X_D (i : fregidx) : SailM (BitVec 64) := do
  assert (flen ≥b 64) "model/extensions/FD/fdext_regs.sail:289.19-289.20"
  throw Error.Exit

def wF_or_X_H (i : fregidx) (data : (BitVec 16)) : SailM Unit := do
  assert (flen ≥b 16) "model/extensions/FD/fdext_regs.sail:304.19-304.20"
  assert (neq_bool (hartSupports Ext_F) (hartSupports Ext_Zfinx)) "model/extensions/FD/fdext_regs.sail:305.55-305.56"
  if ((hartSupports Ext_F) : Bool)
  then (wF_H i data)
  else (wX_bits (fregidx_to_regidx i) (sign_extend (m := 32) data))

def wF_or_X_S (i : fregidx) (data : (BitVec 32)) : SailM Unit := do
  assert (flen ≥b 32) "model/extensions/FD/fdext_regs.sail:313.19-313.20"
  assert (neq_bool (hartSupports Ext_F) (hartSupports Ext_Zfinx)) "model/extensions/FD/fdext_regs.sail:314.55-314.56"
  if ((hartSupports Ext_F) : Bool)
  then (wF_S i data)
  else (wX_bits (fregidx_to_regidx i) (sign_extend (m := 32) data))

def wF_or_X_D (i : fregidx) (data : (BitVec 64)) : SailM Unit := do
  assert (flen ≥b 64) "model/extensions/FD/fdext_regs.sail:322.20-322.21"
  throw Error.Exit
