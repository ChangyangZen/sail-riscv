...
/-- Type quantifiers: index : Nat, 0 ≤ index ∧ index ≤ 63 -/
def write_TLB (index : Nat) (entry : TLB_Entry) : SailM Unit := do
  writeReg tlb (vectorUpdate (← readReg tlb) index (some entry))

def match_TLB_Entry (ent : TLB_Entry) (asid : (BitVec (if ( 32 = 32  : Bool) then 9 else 16))) (vpn : (BitVec (57 - 12))) : Bool :=
  ((ent.global || (ent.asid == asid)) && (ent.vpn == (vpn &&& (Complement.complement ent.levelMask))))

def flush_TLB_Entry (ent : TLB_Entry) (asid : (Option (BitVec (if ( 32 = 32  : Bool) then 9 else 16)))) (vaddr : (Option (BitVec 32))) : Bool :=
...
/-- Type quantifiers: sv_width : Nat, is_sv_mode(sv_width) -/
def lookup_TLB (sv_width : Nat) (asid : (BitVec (if ( 32 = 32  : Bool) then 9 else 16))) (vpn : (BitVec (sv_width - 12))) : SailM (Option TLB_Entry) := do
  ...
  | .some entry =>
    ...
    then (pure (some entry))
    else (pure none))

/-- Type quantifiers: k_ex154063_ : Bool, level : Nat, sv_width : Nat, is_sv_mode(sv_width), 0 ≤
  level ∧
  level ≤
  (if ( sv_width = 32  : Bool) then 1 else (if ( sv_width = 39  : Bool) then 2 else (if ( sv_width =
  48  : Bool) then 3 else 4))) -/
def add_to_TLB (sv_width : Nat) (asid : (BitVec (if ( 32 = 32  : Bool) then 9 else 16))) (vpn : (BitVec (sv_width - 12))) (ppn : (BitVec (if ( sv_width
