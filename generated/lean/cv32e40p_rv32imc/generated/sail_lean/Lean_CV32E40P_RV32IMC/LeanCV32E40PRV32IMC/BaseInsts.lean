...
def jump_to (target : (BitVec 32)) : SailM ExecutionResult := SailME.run do
  match (ext_control_check_pc target) with
  | .some e => SailME.throw ((Ext_ControlAddr_Check_Failure e) : ExecutionResult)
  | none => (pure ())
  assert ((BitVec.access target 0) == 0#1) "model/extensions/I/base_insts.sail:59.29-59.30"
...
/-- Type quantifiers: k_ex154661_ : Bool, width : Nat, width ∈ {1, 2, 4, 8} -/
def valid_load_encdec (width : Nat) (is_unsigned : Bool) : Bool :=
  ((width <b xlen_bytes) || ((not is_unsigned) && (width ≤b xlen_bytes)))

/-- Type quantifiers: k_ex154664_ : Bool, k_n : Nat, k_n ≥ 0, 0 < k_n ∧ k_n ≤ xlen -/
...
/-- Type quantifiers: k_ex154665_ : Bool -/
...
/-- Type quantifiers: k_ex154666_ : Bool -/
