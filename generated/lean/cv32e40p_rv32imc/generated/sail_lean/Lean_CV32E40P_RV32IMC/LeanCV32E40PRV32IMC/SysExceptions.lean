...
def prepare_trap_vector (p : Privilege) (cause : (BitVec 32)) : SailM (BitVec 32) := do
  let tvec ← (( do
    ...
    | User => (internal_error "model/exceptions/sys_exceptions.sail" 25 "Invalid privilege level")
    | VirtualUser =>
      (internal_error "model/exceptions/sys_exceptions.sail" 26 "Hypervisor extension not supported")
    | VirtualSupervisor =>
      (internal_error "model/exceptions/sys_exceptions.sail" 27 "Hypervisor extension not supported")
    ) : SailM Mtvec )
  match (tvec_addr tvec cause) with
  | .some epc => (pure epc)
  | none => (internal_error "model/exceptions/sys_exceptions.sail" 31 "Invalid tvec mode")

def get_xepc (p : Privilege) : SailM (BitVec 32) := do
  match p with
  | Machine => (align_pc (← readReg mepc))
  | Supervisor => (align_pc (← readReg sepc))
  | User => (internal_error "model/exceptions/sys_exceptions.sail" 46 "Invalid privilege level")
  | VirtualUser =>
    (internal_error "model/exceptions/sys_exceptions.sail" 47 "Hypervisor extension not supported")
  | VirtualSupervisor =>
    (internal_error "model/exceptions/sys_exceptions.sail" 48 "Hypervisor extension not supported")
...
def set_xepc (p : Privilege) (value : (BitVec 32)) : SailM (BitVec 32) := do
  ...
  | User => (internal_error "model/exceptions/sys_exceptions.sail" 57 "Invalid privilege level")
  | VirtualUser =>
    (internal_error "model/exceptions/sys_exceptions.sail" 58 "Hypervisor extension not supported")
  | VirtualSupervisor =>
    (internal_error "model/exceptions/sys_exceptions.sail" 59 "Hypervisor extension not supported")
  (pure target)
