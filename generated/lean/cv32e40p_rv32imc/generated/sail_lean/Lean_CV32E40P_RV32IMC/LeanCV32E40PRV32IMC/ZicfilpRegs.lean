...
def zicfilp_preserve_elp_on_trap (x : Privilege) : SailM Unit := do
  ...
  | User => (internal_error "model/extensions/cfi/zicfilp_regs.sail" 71 "Invalid privilege level")
  | VirtualSupervisor =>
    (internal_error "model/extensions/cfi/zicfilp_regs.sail" 72 "Hypervisor extension not supported")
  | VirtualUser =>
    (internal_error "model/extensions/cfi/zicfilp_regs.sail" 73 "Hypervisor extension not supported")
  (reset_elp ())
