...
def within_phys_mem (typ_0 : physaddr) (width : Int) : SailM Bool := do
  ...
  let ram_base_int ← do (pure (BitVec.toNatInt (← readReg plat_ram_base)))
  let rom_base_int ← do (pure (BitVec.toNatInt (← readReg plat_rom_base)))
  let ram_size_int ← do (pure (BitVec.toNatInt (← readReg plat_ram_size)))
  let rom_size_int ← do (pure (BitVec.toNatInt (← readReg plat_rom_size)))
...
def within_clint (typ_0 : physaddr) (width : Nat) : SailM Bool := do
  let .Physaddr addr : physaddr := typ_0
  let addr_int := (BitVec.toNatInt addr)
  let clint_base_int ← do (pure (BitVec.toNatInt (← readReg plat_clint_base)))
  let clint_size_int ← do (pure (BitVec.toNatInt (← readReg plat_clint_size)))
...
def htif_load (acc : (AccessType Unit)) (app_1 : physaddr) (width : Nat) : SailM (Result (BitVec (8 * width)) ExceptionType) := do
  ...
  let base ← (( do
    match (← readReg htif_tohost_base) with
    | .some base => (pure base)
    | none => (internal_error "model/sys/platform.sail" 321 "HTIF load while HTIF isn't enabled") )
    : SailM physaddrbits )
...
def htif_store (app_0 : physaddr) (width : Nat) (data : (BitVec (8 * width))) : SailM (Result Bool ExceptionType) := SailME.run do
  ...
  let base ← (( do
    match (← readReg htif_tohost_base) with
    | .some base => (pure base)
    | none => (internal_error "model/sys/platform.sail" 345 "HTIF store while HTIF isn't enabled") )
    ...
    (do
      ...
      | 0x01 =>
        (do
          ...
          (reset_htif ()))
      | d => (pure (print (HAppend.hAppend "htif-???? cmd: " (BitVec.toFormatted data)))))
  else (pure ())
  (pure (Ok true))
