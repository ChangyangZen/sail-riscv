...
import LeanRV64DFull.Flen
import LeanRV64DFull.Vlen
import LeanRV64DFull.FdextRegs
import LeanRV64DFull.VextRegs
import LeanRV64DFull.FextInsts
import LeanRV64DFull.DextInsts
import LeanRV64DFull.ZfhInsts
...
def get_scalar_fp (rs1 : fregidx) (SEW : Nat) : SailM (BitVec SEW) := do
  ...
  match SEW with
  | 16 => (pure (to_bits_truncate (l := SEW) (BitVec.toNatInt (← (rF_H rs1)))))
  | 32 => (pure (to_bits_truncate (l := SEW) (BitVec.toNatInt (← (rF_S rs1)))))
  | _ => (pure (to_bits_truncate (l := SEW) (BitVec.toNatInt (← (rF_D rs1)))))
...
/-- Type quantifiers: k_ex594451_ : Bool, k_m : Nat, k_m ≥ 0, k_m ∈ {16, 32, 64} -/
def rsqrt7 (v : (BitVec k_m)) (sub : Bool) : SailM (BitVec 64) := do
  ...
  let (normalized_exp, normalized_sig) ← do
    ...
    then
      (do
        let nr_leadingzeros := ((BitVec.countLeadingZeros sig) -i (64 -i s))
        assert (nr_leadingzeros ≥b 0) "extensions/V/vext_fp_utils_insts.sail:478.35-478.36"
        (pure ((to_bits_unsafe (l := 64) (0 -i nr_leadingzeros)), (shiftl sig (1 +i nr_leadingzeros)))))
    else (pure (exp, sig))
  let idx : Nat :=
    match (Sail.BitVec.length v) with
    | 16 =>
  ...
  let out_sig := ((GetElem?.getElem! table (127 -i idx)) *i (2 ^i (s -i 7)))
  let out_exp :=
    (Int.tdiv (((3 *i ((2 ^i (e -i 1)) -i 1)) -i 1) -i (BitVec.toInt normalized_exp)) 2)
  (pure (to_bits_truncate (l := 64)
      ((((BitVec.toNatInt sign) *i (2 ^i (e +i s))) +i (out_exp *i (2 ^i s))) +i out_sig)))

def riscv_f16Rsqrte7 (rm : (BitVec 3)) (v : (BitVec 16)) : SailM ((BitVec 5) × (BitVec 16)) := do
  match (fp_class v) with
  | 0x0001 => (pure ((nvFlag ()), 0x7E00#16))
  | 0x0002 => (pure ((nvFlag ()), 0x7E00#16))
  | 0x0004 => (pure ((nvFlag ()), 0x7E00#16))
  | 0x0100 => (pure ((nvFlag ()), 0x7E00#16))
  | 0x0200 => (pure ((zeros (n := 5)), 0x7E00#16))
  | 0x0008 => (pure ((dzFlag ()), 0xFC00#16))
  | 0x0010 => (pure ((dzFlag ()), 0x7C00#16))
  | 0x0080 => (pure ((zeros (n := 5)), 0x0000#16))
  | 0x0020 => (pure ((zeros (n := 5)), (Sail.BitVec.extractLsb (← (rsqrt7 v true)) 15 0)))
  | _ => (pure ((zeros (n := 5)), (Sail.BitVec.extractLsb (← (rsqrt7 v false)) 15 0)))

def riscv_f32Rsqrte7 (rm : (BitVec 3)) (v : (BitVec 32)) : SailM ((BitVec 5) × (BitVec 32)) := do
  match (Sail.BitVec.extractLsb (fp_class v) 15 0) with
  | 0x0001 => (pure ((nvFlag ()), 0x7FC00000#32))
  | 0x0002 => (pure ((nvFlag ()), 0x7FC00000#32))
  | 0x0004 => (pure ((nvFlag ()), 0x7FC00000#32))
  | 0x0100 => (pure ((nvFlag ()), 0x7FC00000#32))
  | 0x0200 => (pure ((zeros (n := 5)), 0x7FC00000#32))
  | 0x0008 => (pure ((dzFlag ()), 0xFF800000#32))
  | 0x0010 => (pure ((dzFlag ()), 0x7F800000#32))
  | 0x0080 => (pure ((zeros (n := 5)), 0x00000000#32))
  | 0x0020 => (pure ((zeros (n := 5)), (Sail.BitVec.extractLsb (← (rsqrt7 v true)) 31 0)))
  | _ => (pure ((zeros (n := 5)), (Sail.BitVec.extractLsb (← (rsqrt7 v false)) 31 0)))

def riscv_f64Rsqrte7 (rm : (BitVec 3)) (v : (BitVec 64)) : SailM ((BitVec 5) × (BitVec 64)) := do
  match (Sail.BitVec.extractLsb (fp_class v) 15 0) with
  | 0x0001 => (pure ((nvFlag ()), 0x7FF8000000000000#64))
  | 0x0002 => (pure ((nvFlag ()), 0x7FF8000000000000#64))
  | 0x0004 => (pure ((nvFlag ()), 0x7FF8000000000000#64))
  | 0x0100 => (pure ((nvFlag ()), 0x7FF8000000000000#64))
  | 0x0200 => (pure ((zeros (n := 5)), 0x7FF8000000000000#64))
  | 0x0008 => (pure ((dzFlag ()), 0xFFF0000000000000#64))
  | 0x0010 => (pure ((dzFlag ()), 0x7FF0000000000000#64))
  | 0x0080 => (pure ((zeros (n := 5)), (zeros (n := 64))))
  | 0x0020 => (pure ((zeros (n := 5)), (Sail.BitVec.extractLsb (← (rsqrt7 v true)) 63 0)))
  | _ => (pure ((zeros (n := 5)), (Sail.BitVec.extractLsb (← (rsqrt7 v false)) 63 0)))

/-- Type quantifiers: k_ex594664_ : Bool, k_m : Nat, k_m ≥ 0, k_m ∈ {16, 32, 64} -/
def recip7 (v : (BitVec k_m)) (rm_3b : (BitVec 3)) (sub : Bool) : SailM (Bool × (BitVec 64)) := do
  ...
  let (normalized_exp, normalized_sig) :=
    if (sub : Bool)
    then ((to_bits_unsafe (l := 64) (0 -i nr_leadingzeros)), (shiftl sig (1 +i nr_leadingzeros)))
    else (exp, sig)
  let idx : Nat :=
    match (Sail.BitVec.length v) with
  ...
  let mid_exp : Int := (((2 *i ((2 ^i (e -i 1)) -i 1)) -i 1) -i (BitVec.toInt normalized_exp))
  let mid_sig : Int := ((GetElem?.getElem! table (127 -i idx)) *i (2 ^i (s -i 7)))
  let (out_exp, out_sig) : (Int × Int) :=
    if ((mid_exp == 0) : Bool)
    then (mid_exp, ((Int.tdiv mid_sig 2) +i (2 ^i (s -i 1))))
    else
      (if ((mid_exp == ((2 ^i e) -i 1)) : Bool)
      then (0, ((Int.tdiv mid_sig 4) +i (2 ^i (s -i 2))))
      else (mid_exp, mid_sig))
  if ((sub && (nr_leadingzeros >b 1)) : Bool)
  then
    (if (((rm_3b == 0b001#3) || (((rm_3b == 0b010#3) && (sign == 0#1)) || ((rm_3b == 0b011#3) && (sign == 1#1)))) : Bool)
    then
      (pure (true, (to_bits_truncate (l := 64)
          ((((BitVec.toNatInt sign) *i (2 ^i (e +i s))) +i ((2 *i ((2 ^i (e -i 1)) -i 1)) *i (2 ^i s))) +i ((2 ^i s) -i 1)))))
    else
      (pure (true, (to_bits_truncate (l := 64)
          (((BitVec.toNatInt sign) *i (2 ^i (e +i s))) +i (((2 ^i e) -i 1) *i (2 ^i s)))))))
  else
    (pure (false, (to_bits_truncate (l := 64)
        ((((BitVec.toNatInt sign) *i (2 ^i (e +i s))) +i (out_exp *i (2 ^i s))) +i out_sig))))
...
def riscv_f16Recip7 (rm : (BitVec 3)) (v : (BitVec 16)) : SailM ((BitVec 5) × (BitVec 16)) := do
  ...
  match (fp_class v) with
  | 0x0001 => (pure ((zeros (n := 5)), 0x8000#16))
  | 0x0080 => (pure ((zeros (n := 5)), 0x0000#16))
  | 0x0008 => (pure ((dzFlag ()), 0xFC00#16))
  | 0x0010 => (pure ((dzFlag ()), 0x7C00#16))
  | 0x0100 => (pure ((nvFlag ()), 0x7E00#16))
...
def riscv_f32Recip7 (rm : (BitVec 3)) (v : (BitVec 32)) : SailM ((BitVec 5) × (BitVec 32)) := do
  ...
  let (round_abnormal_false, res_false) ← do (recip7 v rm false)
  match (Sail.BitVec.extractLsb (fp_class v) 15 0) with
  | 0x0001 => (pure ((zeros (n := 5)), 0x80000000#32))
  | 0x0080 => (pure ((zeros (n := 5)), 0x00000000#32))
  | 0x0008 => (pure ((dzFlag ()), 0xFF800000#32))
  | 0x0010 => (pure ((dzFlag ()), 0x7F800000#32))
  | 0x0100 => (pure ((nvFlag ()), 0x7FC00000#32))
...
def riscv_f64Recip7 (rm : (BitVec 3)) (v : (BitVec 64)) : SailM ((BitVec 5) × (BitVec 64)) := do
  ...
  let (round_abnormal_false, res_false) ← do (recip7 v rm false)
  match (Sail.BitVec.extractLsb (fp_class v) 15 0) with
  | 0x0001 => (pure ((zeros (n := 5)), 0x8000000000000000#64))
  | 0x0080 => (pure ((zeros (n := 5)), 0x0000000000000000#64))
  | 0x0008 => (pure ((dzFlag ()), 0xFFF0000000000000#64))
  | 0x0010 => (pure ((dzFlag ()), 0x7FF0000000000000#64))
  | 0x0100 => (pure ((nvFlag ()), 0x7FF8000000000000#64))
  | 0x0200 => (pure ((zeros (n := 5)), 0x7FF8000000000000#64))
