...
import LeanRV64DFull.Regs
import LeanRV64DFull.SysRegs
import LeanRV64DFull.ExtRegs
import LeanRV64DFull.SysExceptions
import LeanRV64DFull.PmpRegs
import LeanRV64DFull.FdextRegs
import LeanRV64DFull.VextRegs
import LeanRV64DFull.Smcntrpmf
import LeanRV64DFull.SysControl
import LeanRV64DFull.Platform
import LeanRV64DFull.InstRetire
import LeanRV64DFull.Vmem
import LeanRV64DFull.ZkrControl
  ...
  | 0x310 =>
    (do
      ...
      then (pure (Sail.BitVec.extractLsb (← readReg mstatus) 63 32))
        ...
        (do
          let v__3852 := 0x310#12
          if ((((Sail.BitVec.extractLsb v__3852 11 4) == (0x3A#8 : (BitVec 8))) && (let idx : (BitVec 4) :=
                 (Sail.BitVec.extractLsb v__3852 3 0)
                            ...
                            (do
                              ...
                              | v__3852 =>
                                (do
                                  ...
                                  then
                                    (do
                                      let index : (BitVec 5) := (Sail.BitVec.extractLsb v__3852 4 0)
                                      (read_mhpmevent (← (hpmidx_from_bits index))))
                                  else
                                    (do
                                      if ((((Sail.BitVec.extractLsb v__3852 11 5) == (0b1011000#7 : (BitVec 7))) && (let index : (BitVec 5) :=
        ...
        (do
          writeReg mstatus (← (legalize_mstatus (← readReg mstatus)
              ((Sail.BitVec.extractLsb (← readReg mstatus) 63 32) ++ (Sail.BitVec.extractLsb value
                  31 0))))
          (pure (Ok (zero_extend (m := 64) (Sail.BitVec.extractLsb (← readReg mstatus) 31 0))))))
  | (0x310, value) =>
    (do
      if ((xlen == 32) : Bool)
      then
        (do
          writeReg mstatus (← (legalize_mstatus (← readReg mstatus)
              ((Sail.BitVec.extractLsb value 31 0) ++ (Sail.BitVec.extractLsb (← readReg mstatus)
                  31 0))))
          (pure (Ok (zero_extend (m := 64) (Sail.BitVec.extractLsb (← readReg mstatus) 63 32)))))
      else
        (do
          match (0x310#12, value) with
          | (v__3874, value) =>
            (do
              if ((((Sail.BitVec.extractLsb v__3874 11 4) == (0x3A#8 : (BitVec 8))) && (let idx : (BitVec 4) :=
                     (Sail.BitVec.extractLsb v__3874 3 0)
                   (((BitVec.access idx 0) == 0#1) || (xlen == 32)))) : Bool)
              then
                (do
                  let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__3874 3 0)
                  let idx := (BitVec.toNatInt idx)
                  (pmpWriteCfgReg idx value)
                  (pure (Ok (← (pmpReadCfgReg idx)))))
              else
                (do
                  if (((Sail.BitVec.extractLsb v__3874 11 4) == (0x3B#8 : (BitVec 8))) : Bool)
                  then
                    (do
                      let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__3874 3 0)
                    ...
                    (do
                      if (((Sail.BitVec.extractLsb v__3874 11 4) == (0x3C#8 : (BitVec 8))) : Bool)
                      then
                        (do
                          let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__3874 3 0)
                        ...
                        (do
                          if (((Sail.BitVec.extractLsb v__3874 11 4) == (0x3D#8 : (BitVec 8))) : Bool)
                          then
                            (do
                              let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__3874 3 0)
                            ...
                            (do
                              if (((Sail.BitVec.extractLsb v__3874 11 4) == (0x3E#8 : (BitVec 8))) : Bool)
                              then
                                (do
                                  let idx : (BitVec 4) := (Sail.BitVec.extractLsb v__3874 3 0)
                                    ...
                                    (do
                                      ...
                                      then
                                        (do
                                          ...
                                          (pure (Ok
                                              (zero_extend (m := 64)
                                                (Sail.BitVec.extractLsb (← readReg mcyclecfg) 63
                                                  32)))))
                                      else
                                        (do
                                          ...
                                          | (v__3874, value) =>
                                            (do
                                              if ((((Sail.BitVec.extractLsb v__3874 11 5) == (0b0011001#7 : (BitVec 7))) && (let index : (BitVec 5) :=
                                                     (Sail.BitVec.extractLsb v__3874 4 0)
                                                   ((BitVec.toNatInt index) ≥b 3) : Bool)) : Bool)
                                              then
                                                (do
                                                  let index : (BitVec 5) :=
                                                ...
                                                (do
                                                  if ((((Sail.BitVec.extractLsb v__3874 11 5) == (0b1011000#7 : (BitVec 7))) && (let index : (BitVec 5) :=
                                                         (Sail.BitVec.extractLsb v__3874 4 0)
                                                       ((BitVec.toNatInt index) ≥b 3) : Bool)) : Bool)
                                                  then
                                                    (do
                                                    ...
                                                    (do
                                                      if ((((Sail.BitVec.extractLsb v__3874 11 5) == (0b1011100#7 : (BitVec 7))) && (let index : (BitVec 5) :=
                                                             (Sail.BitVec.extractLsb v__3874 4 0)
                                                           ((xlen == 32) && (((BitVec.toNatInt index) ≥b 3) : Bool)))) : Bool)
                                                      then
                                                        (do
                                                        ...
                                                        (do
                                                          if ((((Sail.BitVec.extractLsb v__3874 11 5) == (0b0111001#7 : (BitVec 7))) && (let index : (BitVec 5) :=
                                                                 (Sail.BitVec.extractLsb v__3874 4 0)
                                                               ((xlen == 32) && (((BitVec.toNatInt
                                                                     index) ≥b 3) : Bool)))) : Bool)
                                                          then
                                        ...
                                        (do
                                          writeReg minstretcfg (← (legalize_smcntrpmf
                                              ...
                                              ((Sail.BitVec.extractLsb (← readReg minstretcfg) 63
                                                  32) ++ (Sail.BitVec.extractLsb value 31 0))))
                                          (pure (Ok
                                              (zero_extend (m := 64)
                                                (Sail.BitVec.extractLsb (← readReg minstretcfg)
                                                  (xlen -i 1) 0))))))
        ...
        (do
          writeReg mseccfg (← (legalize_mseccfg (← readReg mseccfg)
              ((Sail.BitVec.extractLsb (← readReg mseccfg) 63 32) ++ (Sail.BitVec.extractLsb value
                  31 0))))
          (pure (Ok (zero_extend (m := 64) (Sail.BitVec.extractLsb (← readReg mseccfg) 31 0)))))
      else
        (do
  ...
  | (0x757, value) =>
    (do
      ...
      then
        (do
          writeReg mseccfg (← (legalize_mseccfg (← readReg mseccfg)
              ((Sail.BitVec.extractLsb value 31 0) ++ (Sail.BitVec.extractLsb (← readReg mseccfg)
                  31 0))))
          (pure (Ok (zero_extend (m := 64) (Sail.BitVec.extractLsb (← readReg mseccfg) 63 32)))))
      else
        (do
        ...
        (do
          writeReg menvcfg (← (legalize_menvcfg (← readReg menvcfg)
              ((Sail.BitVec.extractLsb (← readReg menvcfg) 63 32) ++ (Sail.BitVec.extractLsb value
                  31 0))))
          (pure (Ok (zero_extend (m := 64) (Sail.BitVec.extractLsb (← readReg menvcfg) 31 0)))))
      else
        (do
  ...
  | (0x31A, value) =>
    (do
      ...
      then
        (do
          writeReg menvcfg (← (legalize_menvcfg (← readReg menvcfg)
              ((Sail.BitVec.extractLsb value 31 0) ++ (Sail.BitVec.extractLsb (← readReg menvcfg)
                  31 0))))
          (pure (Ok (zero_extend (m := 64) (Sail.BitVec.extractLsb (← readReg menvcfg) 63 32)))))
      else
        (do
        ...
        (do
          writeReg medeleg (legalize_medeleg (← readReg medeleg)
            ((Sail.BitVec.extractLsb (← readReg medeleg) 63 32) ++ (Sail.BitVec.extractLsb value
                31 0)))
          (pure (Ok (zero_extend (m := 64) (Sail.BitVec.extractLsb (← readReg medeleg) 31 0))))))
  | (0x312, value) =>
    (do
      if ((xlen == 32) : Bool)
      then
        (do
          writeReg medeleg (legalize_medeleg (← readReg medeleg)
            ((Sail.BitVec.extractLsb value 31 0) ++ (Sail.BitVec.extractLsb (← readReg medeleg) 31
                0)))
          (pure (Ok (zero_extend (m := 64) (Sail.BitVec.extractLsb (← readReg medeleg) 63 32)))))
      else
        (do
                        ...
                        (do
                          ...
                          | (v__3874, value) =>
                            (do
                              ...
                              then
                                (do
                                  let index : (BitVec 5) := (Sail.BitVec.extractLsb v__3874 4 0)
                                  let index ← do (hpmidx_from_bits index)
                                  (write_mhpmevent index value)
                                  (pure (Ok (← (read_mhpmevent index)))))
                              else
                                      ...
                                      then
                                        (do
                                          ...
                                          (write_mhpmcounterh index
                                            (Sail.BitVec.extractLsb value 31 0))
                                          (pure (Ok
                                              (zero_extend (m := 64) (← (read_mhpmcounterh index))))))
                                      else
                                        (do
                                          if ((((Sail.BitVec.extractLsb v__3874 11 5) == (0b0111001#7 : (BitVec 7))) && (let index : (BitVec 5) :=
                                                 (Sail.BitVec.extractLsb v__3874 4 0)
...
/-- Type quantifiers: k_ex605387_ : Bool -/
