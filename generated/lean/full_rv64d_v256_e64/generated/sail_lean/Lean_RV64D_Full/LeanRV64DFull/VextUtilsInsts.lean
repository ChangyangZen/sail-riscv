...
import LeanRV64DFull.Arithmetic
...
/-- Type quantifiers: i : Nat, LMUL_pow : Int, num_elem : Nat, num_elem > 0, 0 ≤ i -/
def is_active_vector_element (num_elem : Nat) (LMUL_pow : Int) (vm : (BitVec 1)) (i : Nat) : SailM (Result Bool Unit) := SailME.run do
  assert (num_elem ≤b vlen) "extensions/V/vext_utils_insts.sail:228.25-228.26"
  let start_element ← (( do
    match (← (get_start_element ())) with
    | .Ok v => (pure v)
    | .Err () => SailME.throw ((Err ()) : (Result Bool Unit)) ) : SailME (Result Bool Unit) Nat )
  let end_element ← do (get_end_element ())
  let real_num_elem :=
    if ((LMUL_pow ≥b 0) : Bool)
    then num_elem
    else (Int.tdiv num_elem (2 ^i (0 -i LMUL_pow)))
  assert (num_elem ≥b real_num_elem) "extensions/V/vext_utils_insts.sail:235.34-235.35"
      ...
      then (pure (Ok false))
        ...
        (do
          ...
          then (pure (Ok false))
            ...
            (do
              if (((vm == 0#1) && ((BitVec.access (← (rV_bits zvreg)) i) == 0#1)) : Bool)
              then (pure (Ok false))
              else (pure (Ok true)))))

/-- Type quantifiers: i : Nat, LMUL_pow : Int, num_elem : Nat, num_elem > 0, 0 ≤ i -/
def is_body_vector_element (num_elem : Nat) (LMUL_pow : Int) (i : Nat) : SailM (Result Bool Unit) := SailME.run do
  assert (num_elem ≤b vlen) "extensions/V/vext_utils_insts.sail:246.25-246.26"
  let start_element ← (( do
    match (← (get_start_element ())) with
    | .Ok v => (pure v)
    | .Err () => SailME.throw ((Err ()) : (Result Bool Unit)) ) : SailME (Result Bool Unit) Nat )
  ...
  assert (num_elem ≥b real_num_elem) "extensions/V/vext_utils_insts.sail:253.34-253.35"
  if ((i <b start_element) : Bool)
  then (pure (Ok false))
  else
    (if ((i >b end_element) : Bool)
...
def init_masked_result (num_elem : Nat) (EEW : Nat) (LMUL_pow : Int) (vd_val : (Vector (BitVec EEW) num_elem)) (vm_val : (BitVec num_elem)) : SailM (Result ((Vector (BitVec EEW) num_elem) × (BitVec num_elem)) Unit) := SailME.run do
  ...
  assert (num_elem ≥b real_num_elem) "extensions/V/vext_utils_insts.sail:282.34-282.35"
  let (mask, result) ← (( do
    let loop_i_lower := 0
...
def init_masked_source (num_elem : Nat) (LMUL_pow : Int) (vm_val : (BitVec num_elem)) : SailM (Result (BitVec num_elem) Unit) := SailME.run do
  ...
  assert (num_elem ≥b real_num_elem) "extensions/V/vext_utils_insts.sail:334.34-334.35"
  let mask ← (( do
    let loop_i_lower := 0
    let loop_i_upper := (num_elem -i 1)
...
def init_masked_result_carry (num_elem : Nat) (EEW : Nat) (LMUL_pow : Int) (vd_val : (BitVec num_elem)) : SailM (Result ((BitVec num_elem) × (BitVec num_elem)) Unit) := SailME.run do
  ...
  assert (num_elem ≥b real_num_elem) "extensions/V/vext_utils_insts.sail:372.34-372.35"
  let (mask, result) ← (( do
    let loop_i_lower := 0
    let loop_i_upper := (num_elem -i 1)
    let mut loop_vars := (mask, result)
...
def init_masked_result_cmp (num_elem : Nat) (EEW : Nat) (LMUL_pow : Int) (vd_val : (BitVec num_elem)) (vm_val : (BitVec num_elem)) : SailM (Result ((BitVec num_elem) × (BitVec num_elem)) Unit) := SailME.run do
  ...
  assert (num_elem ≥b real_num_elem) "extensions/V/vext_utils_insts.sail:412.34-412.35"
  let (mask, result) ← (( do
    let loop_i_lower := 0
    let loop_i_upper := (num_elem -i 1)
    let mut loop_vars := (mask, result)
...
def get_shift_amount (bit_val : (BitVec k_n)) (SEW : Nat) : SailM Nat := do
  ...
  assert ((0 <b lowlog2bits) && (lowlog2bits <b (Sail.BitVec.length bit_val))) "extensions/V/vext_utils_insts.sail:469.43-469.44"
  (pure (Int.tmod (BitVec.toNatInt bit_val) (2 ^i lowlog2bits)))
...
def get_fixed_rounding_incr (vec_elem : (BitVec k_m)) (shift_amount : Nat) : SailM (BitVec 1) := do
  ...
  then (pure 0#1)
    ...
    (do
      ...
      assert (shift_amount <b (Sail.BitVec.length vec_elem)) "extensions/V/vext_utils_insts.sail:480.28-480.29"
      let elem_int := (BitVec.toNatInt vec_elem)
      let bit_below := ((Int.tmod (Int.tdiv elem_int (2 ^i (shift_amount -i 1))) 2) == 1)
      let bit_at := ((Int.tmod (Int.tdiv elem_int (2 ^i shift_amount)) 2) == 1)
      let lower_below_nonzero :=
        if ((shift_amount == 1) : Bool)
        then false
        else ((Int.tmod elem_int (2 ^i (shift_amount -i 1))) != 0)
      let lower_nonzero := ((Int.tmod elem_int (2 ^i shift_amount)) != 0)
      match rounding_mode with
      | 0b00 => (pure (bool_to_bits bit_below))
      | 0b01 => (pure (bool_to_bits (bit_below && (lower_below_nonzero || bit_at))))
      | 0b10 => (pure 0#1)
      | _ => (pure (bool_to_bits ((not bit_at) && lower_nonzero))))

/-- Type quantifiers: len : Nat, k_n : Nat, k_n ≥ len ∧ len > 1 -/
def unsigned_saturation (len : Nat) (elem : (BitVec k_n)) : SailM (BitVec len) := do
  let max := ((2 ^i len) -i 1)
  if (((BitVec.toNatInt elem) >b max) : Bool)
  then
    (do
      writeReg vcsr (Sail.BitVec.updateSubrange (← readReg vcsr) 0 0 1#1)
      (pure (to_bits_truncate (l := len) max)))
  else (pure (to_bits_truncate (l := len) (BitVec.toNatInt elem)))

/-- Type quantifiers: len : Nat, k_n : Nat, k_n ≥ len ∧ len > 1 -/
def signed_saturation (len : Nat) (elem : (BitVec k_n)) : SailM (BitVec len) := do
  let max := ((2 ^i (len -i 1)) -i 1)
  let min := (0 -i (2 ^i (len -i 1)))
  if (((BitVec.toInt elem) >b max) : Bool)
  then
    (do
      writeReg vcsr (Sail.BitVec.updateSubrange (← readReg vcsr) 0 0 1#1)
      (pure (to_bits_truncate (l := len) max)))
    ...
    (do
      if (((BitVec.toInt elem) <b min) : Bool)
      then
        (do
          writeReg vcsr (Sail.BitVec.updateSubrange (← readReg vcsr) 0 0 1#1)
          (pure (to_bits_truncate (l := len) (2 ^i (len -i 1)))))
      else (pure (to_bits_truncate (l := len) (BitVec.toNatInt elem))))
