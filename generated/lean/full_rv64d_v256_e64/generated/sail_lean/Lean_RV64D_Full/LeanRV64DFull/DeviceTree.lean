...
def generate_dts (_ : Unit) : SailM String := do
  ...
  let ram_base_hi ← do (pure (BitVec.toNatInt (shiftr (← readReg plat_ram_base) 32)))
  let ram_base_lo ← do
    (pure (BitVec.toNatInt (Sail.BitVec.extractLsb (← readReg plat_ram_base) 31 0)))
  let ram_size_hi ← do (pure (BitVec.toNatInt (shiftr (← readReg plat_ram_size) 32)))
  let ram_size_lo ← do
    (pure (BitVec.toNatInt (Sail.BitVec.extractLsb (← readReg plat_ram_size) 31 0)))
  let clint_base_hi ← do (pure (BitVec.toNatInt (shiftr (← readReg plat_clint_base) 32)))
  let clint_base_lo ← do
    (pure (BitVec.toNatInt (Sail.BitVec.extractLsb (← readReg plat_clint_base) 31 0)))
  let clint_size_hi ← do (pure (BitVec.toNatInt (shiftr (← readReg plat_clint_size) 32)))
  let clint_size_lo ← do
    (pure (BitVec.toNatInt (Sail.BitVec.extractLsb (← readReg plat_clint_size) 31 0)))
