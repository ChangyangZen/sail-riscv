import LeanRV64DFull.Sail.Sail
import LeanRV64DFull.Sail.BitVec
import LeanRV64DFull.Sail.IntRange
import LeanRV64DFull.Defs
import LeanRV64DFull.Specialization
import LeanRV64DFull.FakeReal
import LeanRV64DFull.RiscvExtras
...
def encdec_mul_op_forwards (arg_ : mul_op) : SailM (BitVec 3) := do
  match arg_ with
  | { high := false, signed_rs1 := true, signed_rs2 := true } => (pure 0b000#3)
  | { high := true, signed_rs1 := true, signed_rs2 := true } => (pure 0b001#3)
  | { high := true, signed_rs1 := true, signed_rs2 := false } => (pure 0b010#3)
  | { high := true, signed_rs1 := false, signed_rs2 := false } => (pure 0b011#3)
  | _ =>
    (do
